...
| 华盛 | 证券交易记录表 Excel；公司行动记录表 Excel（`.xlsx` / `.xls`） | 股票买卖交易流水、现金分红、分红扣费/预扣税参考 | 只上传这两类报税相关表。期初/期末账户资产表仅适合人工核对持仓和现金余额，当前不参与报税计算。 |
...
华盛证券目前按两个 Excel 报表解析，文件名可以带年份，例如 `2025年度证券交易记录表.xlsx` 和 `2025年度公司行动记录表.xlsx`。
...
新手引导会提示富途年度报表、华盛证券交易/公司行动表、长桥/中银国际/方德/盈立/尊嘉等月结单的入口位置，并引导完成上传、解析和生成报告。
...
- 导入富途、长桥、老虎、熊猫、中银国际、方德、盈立、尊嘉、卓锐、华盛、华泰、盈透、永隆等券商材料。
- 自动识别券商和文件类型，减少手动选择。
...
- 支持 2021-2026 年，汇率采用对应年度人民币汇率中间价口径；2026 年在年末汇率公布前暂按 2026-06-30 最新可用中间价折算。
...
flowchart TD
  ...
  K --> L["资本利得RMB = sum(已实现盈亏 × 对应年度汇率)"]
  L --> M["财产转让税基 = max(资本利得RMB, 0)"]
  M --> N["财产转让估算税额 = 税基 × 20%"]

  E3 --> O["股息/利息RMB = 税前收入 × 对应年度汇率"]
  E3 --> P["海外已纳税额RMB = 预扣税 × 对应年度汇率"]
